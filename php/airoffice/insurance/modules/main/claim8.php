...
$sql = "select ov.hn,concat(pt.pname,pt.fname,' ',pt.lname) as fullname,ov.vstdate ,ov.vsttime,ou.name as staff_name ,ptt.name as name_ptt
from ovst ov
left join patient pt on pt.hn=ov.hn
left join pttype ptt ON ov.pttype=ptt.pttype
left join eclaimdb.m_registerdata mr on mr.OPDSeq=ov.vn  
inner join visit_pttype vt on vt.vn=ov.vn
left join ipt_pttype ipy on ipy.an=ov.an
left join vn_stat vns on vns.vn=ov.vn
LEFT OUTER JOIN opduser ou ON ou.loginname=ov.staff
where    ov.vstdate = curdate() and ov.pttype in (41,44,42,43,99,40,52,49,98,97)  and vt.claim_code is null ";
...
if ($num > 0) {
    ...
    foreach ($result AS $row) {
       ...
       $lineapi = "USVOCdeyEnLFLLuevtZRWUKSYBrb2hadrd70kZ9jZQo";
...
//wWhE4ZpvHO40gQzW27J8Vlps5fPyqn785qcK2iW6W6D กลุ่มงานประกัน
        ...
        .$row['vsttime']."  " . $row["fullname"] . "  ". "  สิทธ์:" . $row['name_ptt'] . " " . $row['typename']
        
                        . "  " . $row['hosname'] . "   " . "ผู้ส่งตรวจ:" . $row['staff_name']
