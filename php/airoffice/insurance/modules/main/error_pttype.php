...
class MySqlConn {
    ...
    public function query($sql = '', $tableName = '') {
        ...
        $query = $this->_mysql->query("$sql $this->_tableName $this->_where$this->_order $this->_limit");
...
$sql = "SELECT ov.staff,
            pat.cid,
            hd.name AS hospdep, 
            CONCAT(hp.hosptype,'',hp.name) AS hosname,
            CONCAT(pat.pname,pat.fname,' ',pat.lname) AS fullname,
            vn.vstdate,
            vn.hn,
            vn.vn,
            vn.pttype,
        ...
        WHERE  vn.vstdate BETWEEN '2022-12-15' AND CURDATE()
        AND pt.hipdata_code IN ('UCS','WEL')
        AND vn.pttype NOT IN
            ('29','51','52','40','41',
            '42','43','44','49','13',
            '15','98','56','57','58',
            '59','40','99','17','76',
...
if ($num > 0) {
    ...
    $mms = "\n" .
    "HN : " . $row['hn'] . "\n" .
    "วันที่รับบริการ : " . $row['vstdate'] . "\n" . 
    "เวลา : " . $row['vsttime'] . "\n" .
    $row["fullname"] . "\n" .
    "สิทธ์ : " . "[" . $row['pttype'] . "]" . " " . $row['typename'] . "\n" .
    $row['hosname'] . "\n" .
    "ผู้ส่งตรวจ:" . $row['name_staff']. "  " . $row['staff']
    ;
